...
class Statistics:
    ...
    def __init__(self, loss=0, n_words=0, n_correct=0):
        ...
        self.n_src_words = 0
        # Accumulation from all ranks
        self.loss_acc = 0
        self.n_words_acc = 0
        self.n_correct_acc = 0
        self.n_src_words_acc = 0
    ...
    def accuracy(self):
        self.n_correct_acc = onmt.allReduceScalar(self.n_correct)
        self.n_words_acc = onmt.allReduceScalar(self.n_words)
        return 100 * (self.n_correct_acc / float(self.n_words_acc))

    def ppl(self):
        self.loss_acc = onmt.allReduceScalar(self.loss)
        self.n_words_acc = onmt.allReduceScalar(self.n_words)
        return math.exp(min(self.loss_acc / self.n_words_acc, 100))

    def elapsed_time(self):
        return time.time() - self.start_time

    def output(self, epoch, batch, n_batches, start, optim):
        t = self.elapsed_time()

        self.n_words_acc = onmt.allReduceScalar(self.n_words)
        self.n_src_words_acc = onmt.allReduceScalar(self.n_src_words)

        print(("Epoch %2d, %5d/%d; Optim %s LR %.2f; acc: %6.2f; ppl: %6.2f;" +
              ...
              (epoch, batch,  n_batches,
               ...
               self.accuracy(),
               self.ppl(),
               self.n_src_words_acc / (t + 1e-5),
               self.n_words_acc / (t + 1e-5),
               time.time() - start))
        sys.stdout.flush()
